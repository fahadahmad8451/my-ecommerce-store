...
export function TrackOrderForm() {
  const [values, setValues] = useState<FormValues>({}); const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle"); const [message, setMessage] = useState(""); const [tracking, setTracking] = useState<{ orderName: string; fulfillmentStatus: string; tracking: Array<{ company: string | null; number: string | null; url: string | null }> } | null>(null);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!values.orderNumber?.trim() || !/^\S+@\S+\.\S+$/.test(values.email || "")) { setStatus("error"); setMessage("Enter your order number and a valid order email."); return; } setStatus("loading"); setMessage(""); setTracking(null); try { const response = await fetch("/api/orders/track", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Tracking is currently unavailable."); setTracking(data.tracking); setStatus("success"); setMessage(""); } catch (error) { setStatus("error"); setMessage(error instanceof Error ? error.message : "Tracking is currently unavailable."); } }
  return <form className="support-form" onSubmit={submit} noValidate><label>Order number<input required value={values.orderNumber || ""} onChange={event => setValues(current => ({ ...current, orderNumber: event.target.value }))}/></label><label>Email used for order<input required type="email" autoComplete="email" value={values.email || ""} onChange={event => setValues(current => ({ ...current, email: event.target.value }))}/></label><button className="btn-primary" type="submit" disabled={status === "loading"}>{status === "loading" ? "Checking..." : "Track order"}</button><p className={`form-message ${status}`} aria-live="polite">{message}</p>{tracking && <div className="tracking-result"><strong>{tracking.orderName} · {tracking.fulfillmentStatus}</strong>{tracking.tracking.length ? tracking.tracking.map((item, index) => <p key={`${item.number}-${index}`}>{item.company || "Carrier"}: {item.url ? <a href={item.url} target="_blank" rel="noreferrer">{item.number || "Open tracking"}</a> : item.number || "Tracking pending"}</p>) : <p>Tracking will appear once this order has been fulfilled.</p>}</div>}</form>;
}
